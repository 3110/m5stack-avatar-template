...
int AquesTalkTTS::play(const char *koe, int speed) {
    ...
    if (taskAquesTalk == 0) {
        xTaskCreate(task_TTS_synthe, "task_TTS_synthe", 4096, NULL,
                    TASK_PRIORITY, &taskAquesTalk);
    } else {
...
static i2s_config_t i2s_config = {
    ...
    .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX | I2C_MODE_DAC_BUILT_IN),
#endif
...
static const i2s_pin_config_t pin_config = {
    .bck_io_num = 12, .ws_io_num = 0, .data_out_num = 2, .data_in_num = 34};
#endif

static void DAC_create() {
    AqResample_Reset();
    i2s_driver_install((i2s_port_t)i2s_num, &i2s_config, 0, NULL);
#if defined(TARGET_M5BASIC)
...
static int DAC_write_val(uint16_t val) {
    ...
    esp_err_t iret =
        i2s_write((i2s_port_t)i2s_num, sample, sizeof(uint16_t) * 2,
...
extern "C" size_t aqdic_read(size_t pos, size_t size, void *buf) {
    return reader.read(pos, size, buf);
}
